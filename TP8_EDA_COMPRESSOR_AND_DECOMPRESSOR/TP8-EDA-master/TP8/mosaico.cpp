...
bool mosaico::compress_imgs()
{
	bool comp_error = true;
	string ext = ".ajm";
	for (unsigned int j = 0; j < img_qnt; j++)
	{
		if (imgs_lib[j].is_selected())
		{
			...
			if (!compress_algorithm(imgs_lib[j].get_path(), out_name.c_str(), threshold)) 
			{
				comp_error = false;
			}
			comp_something = true;
...
bool mosaico::is_any_comp()
{
	return comp_something;
}
...
void mosaico::set_thershold(unsigned int th)
{
	threshold = th;
}
