...
imgs_catalog::imgs_catalog(vector<string> &imgs_paths, unsigned int th_data)
{
...
bool imgs_catalog::draw_page(unsigned int index)
{
	if (index < mosaico_qnt)
	{
		...
		al_draw_text(pages[index].get_font(), WHITE, page_compTitle_x, page_compTitle_y,
			ALLEGRO_ALIGN_CENTER, "IMAGE COMPRESSOR");
		al_draw_text(pages[index].get_font(), WHITE, page_compTitle_x, page_compTitle_y + 21,
			ALLEGRO_ALIGN_CENTER, "Select images to compress with 1 - 9");
...
void imgs_catalog::compress_all()
{
	...
	if (!process_error)
	{
		...
		for (unsigned int i = 0; i < mosaico_qnt; i++)
		{
			...
			{
				pages[i].is_any_comp() ? comp_flag = true : false;
			}
		}
		...
		if (comp_flag)
		{
			al_clear_to_color(BLACK);
			al_draw_text(pages[0].get_font(), WHITE, 1000 / 2, 600 / 2, ALLEGRO_ALIGN_CENTER,
				"Compressing complete. Close display to exit.-");
			al_flip_display();
		}
		else
		{
			al_clear_to_color(BLACK);
			al_draw_text(pages[0].get_font(), WHITE, 1000 / 2, 600 / 2, ALLEGRO_ALIGN_CENTER,
				"Nothing to compress selected. Close display to exit.-");
			al_flip_display();
		}
	}
	else
	{
		al_clear_to_color(BLACK);
		al_draw_text(pages[0].get_font(), WHITE, 1000 / 2, 600 / 2, ALLEGRO_ALIGN_CENTER,
			"Some images were not compressed. See CMD for info. Close display to exit.-");
		al_flip_display();
	}
}
