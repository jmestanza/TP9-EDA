...
void color_data::avg_update(int size) {
	if (size == 0) {
		cout << ">> Fatal error in image compression algorithm." << endl;
		return;
	}
	avg_red /= size;
	avg_green /= size;
...
bool compress_algorithm(string image_file,string output_file, int th) {
	...
	if (width != height || (pow2.find(width) == pow2.end())) {
		cout << ">> Imagen no potencia de 2 insertada." << endl;
		return 0;
	}
	string data;
	int v = width;
...
bool uncompress_algorithm(string data_directory, string file_to_write) { // reconstruct image
	...
	if (!file.is_open()) {
		cout << ">> Error opening '" << data_directory << endl;
		file.close();
		return 0;
	}
	string data = "";
	...
	if (!recu_uncompress(image, data, itr, sz, sz, 0, sz - 1, 0, sz - 1)) {
		cout << ">> Uncompress algorithm failed." << endl;
		return 0;
	}
